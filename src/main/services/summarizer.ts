...
async function chat(
  ...
  const doFetch = async (jsonMode: boolean): Promise<string> => {
    ...
    if (!res.ok)
      throw new ApiError(
        res.status,
        `LLM 请求失败 HTTP ${res.status}: ${text.slice(0, 300)}`,
      );
...
function toChapters(data: unknown): Chapter[] {
  ...
  if (!Array.isArray(obj.chapters))
    throw new Error("LLM 返回缺少 chapters 字段");
  return (
    obj.chapters as { title?: unknown; summary?: unknown; points?: unknown }[]
  )
...
const CHUNK_PROMPT = (
  title: string,
  chunk: string,
): string => `你是一位专业的视频内容分析师。以下是视频《${title}》的一段文字稿片段：
...
const FINAL_PROMPT = (
  title: string,
  chapters: Chapter[],
): string => `你是一位专业的视频内容分析师。以下是视频《${title}》各章节的摘要：
...
const SINGLE_PROMPT = (
  title: string,
  transcript: string,
): string => `你是一位专业的视频内容分析师。以下是视频《${title}》的完整文字稿：
...
export async function summarizeTranscript(
  ...
  if (chunks.length <= 1) {
    const data = (await chatJson(
      [
        {
          role: "user",
          content: SINGLE_PROMPT(trimmed, transcript.slice(0, CHUNK_CHARS * 2)),
        },
      ],
      8192,
    )) as {
  ...
  for (let i = 0; i < chunks.length; i++) {
    const data = (await chatJson(
      [{ role: "user", content: CHUNK_PROMPT(trimmed, chunks[i]) }],
      4096,
    )) as {
      chapters?: unknown;
    };
  ...
  const data = (await chatJson(
    [
      {
        role: "user",
        content: FINAL_PROMPT(trimmed, compactChapters(chapterLists.flat())),
      },
    ],
    8192,
  )) as {
